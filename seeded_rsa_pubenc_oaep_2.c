...
// Deterministic RSAES-OAEP (SHA-256 / MGF1-SHA-256) using a custom provider RAND
// seeded from <hex-seed>. Supports optional OAEP label via:
//   --label "ascii text"      OR
//   --label-hex DEADBEEF
//
...
static void print_usage(const char *prog) {
    fprintf(stderr,
        "Usage:\n"
        "  %s [--label \"ascii\"] [--label-hex HEX] <hex-seed> \"message\" public.pem\n"
        "\nNotes:\n"
...
static int parse_args(int argc, char **argv, cli_args *out) {
    ...
    while (i < argc && strncmp(argv[i], "--", 2) == 0) {
        if (strcmp(argv[i], "--label") == 0) {
            ...
            if (!hex2bin(argv[i+1], &buf, &blen)) return 0;
            out->label = buf;
            out->label_len = blen;
            out->label_alloc = 1;
            i += 2;
    ...
    // Remaining positional: <hex-seed> "message" public.pem
    if (i + 2 >= argc) return 0;
    out->seed_hex = argv[i];
    out->message  = argv[i+1];
    out->pub_path = argv[i+2];
    return 1;
}

// =====================================================================================
// main: [--label/--label-hex] <hex-seed> "message" public.pem -> hex ciphertext
// =====================================================================================
...
int main(int argc, char **argv) {
    ...
    if (!hex2bin(args.seed_hex, &seed, &seedlen)) { fprintf(stderr, "Bad hex seed\n"); return 1; }
    ...
    if (!pub) { fprintf(stderr, "Failed to load public key from %s\n", args.pub_path); return 1; }

    // Prepare message
    const unsigned char *msg = (const unsigned char*)args.message;
    size_t mlen = strlen(args.message);
    ...
    if (!oaep_encode_deterministic(rctx, EM, k, msg, mlen, label, label_len, md, mgf1md)) {
    ...
    // Cleanup
    if (args.label_alloc && args.label) OPENSSL_free((void*)args.label);
    OPENSSL_free(EM);
    OPENSSL_free(C);
    EVP_PKEY_free(pub);
